...
Elm.App.make = function (_elm) {
   ...
   var renderEndpoint = function (endpoint) {
      ...
      _U.list([A2($Html.h2,_U.list([]),_U.list([$Html.text(endpoint.name)]))
              ...
              ,A2($Html.p,_U.list([]),_U.list([$Html.text(endpoint.request.uri)]))]))]));
   };
   ...
   var update = F2(function (action,model) {    var _p0 = action;return {ctor: "_Tuple2",_0: A2($Maybe.withDefault,testModel,_p0._0),_1: $Effects.none};});
   var Endpoint = F3(function (a,b,c) {    return {name: a,cdcDisabled: b,request: c};});
   var Request = function (a) {    return {uri: a};};
   var decodeEndpoint = function () {
      var endpoint = A4($Json$Decode.object3,
      Endpoint,
      A2($Json$Decode._op[":="],"Name",$Json$Decode.string),
      A2($Json$Decode._op[":="],"CDCDisabled",$Json$Decode.bool),
      A2($Json$Decode.object1,Request,A2($Json$Decode.at,_U.list(["Request","URI"]),$Json$Decode.string)));
