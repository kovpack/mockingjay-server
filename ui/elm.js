...
Elm.App.make = function (_elm) {
   ...
   var renderEndpoint = function (endpoint) {
      return A2($Html.li,_U.list([]),_U.list([A2($Html.div,_U.list([]),_U.list([A2($Html.h2,_U.list([]),_U.list([$Html.text(endpoint.name)]))]))]));
   };
   ...
   var decodeEndpoint = function () {
      var endpoint = A2($Json$Decode.object1,function (title) {    return {name: title};},A2($Json$Decode._op[":="],"Name",$Json$Decode.string));
   ...
   var testModel = _U.list([{name: "Test endpoint"},{name: "Another endpoint"}]);
   var init = {ctor: "_Tuple2",_0: testModel,_1: getEndpoints};
   var update = F2(function (action,model) {
      var _p0 = action;
      var foo = 1 + A2($Debug.log,"number",1);
      return {ctor: "_Tuple2",_0: A2($Maybe.withDefault,testModel,_p0._0),_1: $Effects.none};
   });
   var app = $StartApp.start({init: init,update: update,view: view,inputs: _U.list([])});
   var main = app.html;
   var tasks = Elm.Native.Task.make(_elm).performSignal("tasks",app.tasks);
   var Endpoint = function (a) {    return {name: a};};
